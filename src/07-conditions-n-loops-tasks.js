...
function findFirstSingleChar(str) {
  const newStr = str;
  let letter = null;
  newStr.split('').reverse().forEach((elem) => {
    if (newStr.split(elem).length - 1 === 1) {
      letter = elem;
    }
  });
  return letter;
...
function isCreditCardNumber(ccn) {
  return ccn.toString().split('').reverse().map((elem, index) => {
    let curr;
    if (index % 2) {
      curr = +elem * 2;

      if (curr > 9) {
        return +curr.toString()[0] + +curr.toString()[1];
      }
      return curr;
    }
    return +elem;
  })
    .reduce((total, curr) => total + curr) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let newNum = num;
  while (newNum > 9) {
    newNum = newNum.toString().split('').reduce((total, curr) => total + +curr, 0);
  }
  return newNum;
}
